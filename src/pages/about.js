...
import { Play, Box, Settings, Building2, Navigation, PlaneTakeoff } from "lucide-react";

const TimelineEvent = ({ year, events, isLeft, icon: Icon }) => (
    <div
        className={`mb-12 flex flex-col md:flex-row justify-between items-center w-full ${isLeft ? "md:flex-row-reverse" : ""
            }`}
    >
        ...
        {/* Event Card */}
        <div
            className={`order-1 bg-white border border-gray-200 rounded-lg shadow-md w-full md:w-5/12 px-6 py-4 ${isLeft ? "md:text-right" : ""
                }`}
        >
            <div className="flex items-start">
                ...
                <div className="flex-1">
                    ...
                    {events.slice(1).map((event, index) => (
                        <p key={index} className="text-left ml-4 text-sm sm:text-base text-gray-700 leading-relaxed mt-2">
                            {event}
                        </p>
                    ))}
                </div>
...
const about = () => {
    const timelineEvents = [
        { year: 2024, events: ["03 Brands Launched", "A timeline is a graphical representation on which important events are marked."], icon: PlaneTakeoff },
        { year: 2023, events: ["Launched 02 Brands for popped grains & Millet puffs - Mobite, Cerelia"], icon: Play },
        ...
        { year: 2020, events: ["New Product Development - Popped grains & extrusion puffing", "Distribution of WoW across Tamil Nadu"], icon: PlaneTakeoff },
    ...
    return (
        <Layout>
            ...
            <div className="px-4 md:px-8 lg:px-16 bg-gradient-to-r from-orange-100 to-yellow-100">
                <div className="border-b-2 border-orange-600 p-2 md:p-4">
                    <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-3 md:mb-4 text-orange-600 text-center -mt-8">OUR MISSION</h2>
                    <p className="text-center text-base md:text-lg lg:text-xl text-black leading-relaxed max-w-3xl mx-auto">
                        <span className="text-xl md:text-2xl lg:text-3xl font-bold text-orange-600">"</span>
            ...
            <div className="py-3 md:py-4 px-4 md:px-8 lg:px-16 bg-gradient-to-r from-orange-100 to-yellow-100">
                <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-3 md:mb-4 text-orange-600 text-center">OUR STORY</h2>
